...
public class Launcher {
  ...
  public static void shootMotorInit() {
    Map.rightLauncher.configSelectedFeedbackSensor(
      TalonSRXFeedbackDevice.CTRE_MagEncoder_Relative,
      0,
      0
    );
    Map.leftLauncher.configSelectedFeedbackSensor(
      TalonSRXFeedbackDevice.CTRE_MagEncoder_Relative,
      0,
      0
    );
  ...
  public static boolean run(double leftTrigger, boolean red) {
    ...
    Map.launcherPivot.set(
      ControlMode.PercentOutput,
      pivotLauncher.calculate(
        Map.launcherPivot.getSelectedSensorPosition(),
        calculatedAngle
      )
    );
    if (
      Math.abs(
        Math.abs(Map.launcherPivot.getSelectedSensorPosition()) -
        Math.abs(calculatedAngle)
      ) <
      800
    ) {
      return true;
